...
Authors / History
-----------------
2013-08 >> Add soap header, when user input param as string. Add passwordDigest type. Add ajax async:false
Emma.Tsai
...
(function($) {
	...
	$.soap = function(options) {
		...
		if ($.type(config.params) === "string") {
			...
			if (/\S/.test(config.params)) {
				...
				soapRequest.toString = function(){
					var finalXml = xml;
					if (false == SOAPTool.isWrappedWithHeader(xml))
					{
						var header = this.getHeader();
						finalXml = xml.replace("><soap:Body",">"+header+"<soap:Body"); 
					}
					return finalXml;
				};
			} else {
		...
		if (!!config.wss && !!config.wss.username && !!config.wss.password) {
			...
			var passType = (config.wss.passwordType=="Digest")?"PasswordDigest":"PasswordText";
			...
			wssPassword.attr('Type', "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#"+passType);
		...
		var clientCallback = function (response) {
				...
				if (response.status !== 'success') {
				    //console.log("[jquery.soap.js] Line 229: Fail response:" + response + ", retry=" + iRetry);
					if (iRetry < 3) //response.httpText == 'NetworkError'
					{
						iRetry++;
						client.SendRequest(action, soapRequest, clientCallback);
					}
					else
					{
				    ...
				    //console.log("[jquery.soap.js] Line 241: Success response:" + response);
	...
	var SOAPTool=(function(){
		var _self = {
			...
			isSOAPHeader: function(xml) {
				return (xml.indexOf(SOAPTool.SOAP_HEADER) !== -1);
			},
			isWrappedWithEnvelope: function(xml) {
				return (this.isSOAP11(xml) || this.isSOAP12(xml));
			},
			isWrappedWithHeader: function(xml) {
				return this.isSOAPHeader(xml);
			},
	...
	var SOAPRequest=function(soapObj) {
		...
		this.getHeader=function(){
			var soapHeader = new SOAPObject("soap:Header");
			var tHdr;
